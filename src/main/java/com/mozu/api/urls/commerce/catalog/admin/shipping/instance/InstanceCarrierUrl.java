
/**
*     This code was auto-generated by a tool.     
*
*     Changes to this file may cause incorrect behavior and will be lost if
*     the code is regenerated.
*/
package com.mozu.api.urls.commerce.catalog.admin.shipping.instance;

import com.mozu.api.utils.UrlFormatter;


public class InstanceCarrierUrl
{

	/**
        * Get Resource Url for GetConfigurations
        * @param filter A set of expressions that consist of a field, operator, and value and represent search parameter syntax when filtering results of a query. Valid operators include equals (eq), does not equal (ne), greater than (gt), less than (lt), greater than or equal to (ge), less than or equal to (le), starts with (sw), or contains (cont). For example - "filter=IsDisplayed+eq+true"
        * @param pageSize Specifies the number of results to display on each page when creating paged results from a query. The maximum value is 200.
        * @param sortBy 
        * @param startIndex 
        * @return    String Resource Url
    */
	public static String getConfigurationsUrl(String filter, Integer pageSize, String sortBy, Integer startIndex)
	{
		UrlFormatter formatter = new UrlFormatter("/api/commerce/catalog/admin/shipping/instance/carriers/?startIndex={startIndex}&pageSize={pageSize}&sortBy={sortBy}&filter={filter}");
		formatter.formatUrl("filter", filter);
		formatter.formatUrl("pageSize", pageSize);
		formatter.formatUrl("sortBy", sortBy);
		formatter.formatUrl("startIndex", startIndex);
		return formatter.getResourceUrl();
	}

	/**
        * Get Resource Url for GetConfiguration
        * @param carrierId Identifier of the shipping carrier associated with the configuration details to retrieve.
        * @return    String Resource Url
    */
	public static String getConfigurationUrl(String carrierId)
	{
		UrlFormatter formatter = new UrlFormatter("/api/commerce/catalog/admin/shipping/instance/carriers/{carrierId}");
		formatter.formatUrl("carrierId", carrierId);
		return formatter.getResourceUrl();
	}

		/**
        * Get Resource Url for CreateConfiguration
        * @param carrierId Identifier of the carrier configuration being created.
        * @return    String Resource Url
    */
	public static String createConfigurationUrl(String carrierId)
	{
		UrlFormatter formatter = new UrlFormatter("/api/commerce/catalog/admin/shipping/instance/carriers/{carrierId}");
		formatter.formatUrl("carrierId", carrierId);
		return formatter.getResourceUrl();
	}

		/**
        * Get Resource Url for UpdateConfiguration
        * @param carrierId Identifier of the shipping carrier configuration to update.
        * @return    String Resource Url
    */
	public static String updateConfigurationUrl(String carrierId)
	{
		UrlFormatter formatter = new UrlFormatter("/api/commerce/catalog/admin/shipping/instance/carriers/{carrierId}");
		formatter.formatUrl("carrierId", carrierId);
		return formatter.getResourceUrl();
	}

		/**
        * Get Resource Url for DeleteConfiguration
        * @param carrierId Identifier of the specific shipping carrier to delete.
        * @return    String Resource Url
    */
	public static String deleteConfigurationUrl(String carrierId)
	{
		UrlFormatter formatter = new UrlFormatter("/api/commerce/catalog/admin/shipping/instance/carriers/{carrierId}");
		formatter.formatUrl("carrierId", carrierId);
		return formatter.getResourceUrl();
	}

	
}

